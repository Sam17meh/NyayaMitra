...
import { createClientLegalPDF } from './pdfService';
...
// Interceptor: Attach Firebase ID Token or demo token to every request header
  ...
  async (config) => {
    try {
      ...
      if (auth && auth.currentUser) {
        ...
        token = sessionStorage.getItem('nyayamitra_demo_user') || 'demo-token';
      }
      if (token) {
...
function getDomainSpecificLegalContent(message, isHindi) {
...
// Response Interceptor: Provide seamless hackathon offline/fallback data ONLY if backend is completely unreachable
api.interceptors.response.use(
  (response) => response,
  async (error) => {
    // If backend connection fails completely (e.g. ECONNREFUSED or 404 in demo mode)
    if (!error.response || error.code === 'ERR_NETWORK' || error.response.status === 404) {
      const url = error.config?.url || '';
      console.warn(`[NyayaMitra API Offline Handler] Generating fallback response for endpoint: ${url}`);

      // Fallback for Chat API: POST /api/chat
      if (url.includes('/api/chat')) {
        let requestBody = {};
        try {
          requestBody = typeof error.config.data === 'string' ? JSON.parse(error.config.data) : (error.config.data || {});
        } catch (e) {
          console.error('Error parsing request body:', e);
        }
        const userMsg = requestBody.message || '';
        const reqLang = requestBody.language || 'English';
        const isHindi = reqLang.toLowerCase() === 'hindi' || reqLang === 'हिन्दी';
        const dInfo = getDomainSpecificLegalContent(userMsg, isHindi);

        const fallbackAnswer = isHindi
          ? `### 📌 1. मामले का मुख्य सारांश और कानूनी ट्राइएज (EXECUTIVE CASE SUMMARY)
- **अवलोकन (Overview)**: आपके द्वारा दर्ज कानूनी समस्या: "${userMsg}" का संपूर्ण विषय-विशेष विश्लेषण (${dInfo.category})।
- **आपातकाल का स्तर (Urgency Level)**: ${dInfo.urgency}
- **प्रमुख कानूनी अधिकार (Key Legal Rights)**: ${dInfo.rights}
...
- **Overview**: Exhaustive domain-matched legal breakdown (${dInfo.category}) for query: "${userMsg}".
- **Urgency Level**: ${dInfo.urgency}
- **Key Legal Rights**: ${dInfo.rights}
...
*Disclaimer: NyayaMitra provides automated AI-driven legal information under Indian Law. Consult your local DLSA panel for court representation.*`;
      ...
      // Fallback for SOS Alert API: POST /api/sos/trigger or /api/sos
      if (url.includes('/api/sos/trigger') || url.includes('/api/sos')) {
        let reqData = {};
        try {
          reqData = typeof error.config.data === 'string' ? JSON.parse(error.config.data) : (error.config.data || {});
        } catch (e) {}

        const userLoc = reqData.locationName || (reqData.city ? `${reqData.city}, ${reqData.state || ''}` : 'Captured Location');
        const stationName = reqData.city ? `${reqData.city} District Central Police Station & Legal Aid Desk` : 'Central District Police Station & Legal Aid Desk';
      ...
      // Fallback for Template PDF Generation API: POST /api/template/generate
      if (url.includes('/api/template/generate')) {
        let requestData = {};
        try {
        ...
        const templateId = requestData.templateId || requestData.templateType || 'fraud_complaint';
        
        try {
          const pdfBlob = await createClientLegalPDF(templateId, requestData);
          return Promise.resolve({ data: pdfBlob });
        } catch (pdfErr) {
          console.error('Client PDF fallback generation failed:', pdfErr);
